...
for (i in indicadores) {
  set.seed(i)
  for (j in 1:n) {
    y[[i]][j] <- rnbinom(1,mu = mu[[i]][j], size = size) #modificando os valores da lista
  }
}
...
write_xlsx(p.coefs_save,"D:/Lucas Akio/.Mestrado/.Simulação/estudo_comparativo/coefs_poisson_NB.xlsx")
...
#0.00437283
MSE_x1_nb <- MSE(nb.coefs_save$`dados[[i]]$V2`,1)
#0.008870212
MSE_x2_nb <- MSE(nb.coefs_save$`dados[[i]]$V3`,-1)
#0.0123525
...
write_xlsx(nb.coefs_save,"D:/Lucas Akio/.Mestrado/.Simulação/estudo_comparativo/coefs_NB_NB.xlsx")
...
write_xlsx(bell.coefs_save,"D:/Lucas Akio/.Mestrado/.Simulação/estudo_comparativo/coefs_bell_NB.xlsx")
...
write_xlsx(twp.coefs_save,"D:/Lucas Akio/.Mestrado/.Simulação/estudo_comparativo/coefs_twp_NB.xlsx")
...
twp.mcglm <- mcglm::mcglm(linear_pred = c(y ~ x1 + x2),
                          matrix_pred = list(mc_id(data = dados)),
                          link = "log",
                          variance = "poisson_tweedie",
                          power_fixed = FALSE,
                          data = dados,
