...
for (i in indicadores) {
  ...
  x1[[i]] = rnorm(n, mean = 0,sd = 1)
  
  for (j in indicadores) {
    
    set.seed(j)
    
    x2[[j]] = rtweedie(n, power = 2, phi = 1, mu = 10) 
  }
...
for (i in indicadores) {
  set.seed(i)
  for (j in 1:n) {
    y[[i]][j] <- rpoistweedie(n = 1,
                              p = 1,
                              mu = mu[[i]][j],
                              lambda = 10, 
                              theta0 = 0) #modificando os valores da lista
  }
}
...
write_xlsx(p.coefs_save,"D:/Lucas Akio/.Mestrado/.Simulação/estudo_comparativo/coefs_poisson_twp.xlsx")
...
write_xlsx(nb.coefs_save,"D:/Lucas Akio/.Mestrado/.Simulação/estudo_comparativo/coefs_NB_twp.xlsx")
...
write_xlsx(bell.coefs_save,"D:/Lucas Akio/.Mestrado/.Simulação/estudo_comparativo/coefs_bell_twp.xlsx")
...
twp.mcglm[[100]]$Covariance

twp.coefs <- NULL
twp.lambda <- NULL
for (i in 1:100) {
  twp.coefs[[i]] <- twp.mcglm[[i]]$Regression
  twp.lambda[[i]] <- twp.mcglm[[i]]$Covariance
}
...
write_xlsx(twp.coefs_save,"D:/Lucas Akio/.Mestrado/.Simulação/estudo_comparativo/coefs_twp_twp.xlsx")
